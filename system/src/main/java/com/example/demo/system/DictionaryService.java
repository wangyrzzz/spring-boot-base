...
public class DictionaryService {
    ...
    public List<Map<String, Object>> list(boolean business, String code, Long parentId) {
        ...
        StringBuilder sql = new StringBuilder("select * from ").append(table).append(" where is_deleted=0");
    ...
    public Map<String, Object> detail(boolean business, Long id) {
        return jdbcTemplate.queryForMap("select * from " + table(business) + " where id=? and is_deleted=0", id);
    }
    ...
    public long save(boolean business, Map<String, Object> input) {
        ...
        if (id == null) {
            jdbcTemplate.update("insert into " + table + " (parent_id, code, dict_key, dict_value, sort, remark, is_sealed, is_deleted, create_time, update_time) values (?,?,?,?,?,?,0,0,current_timestamp,current_timestamp)",
                    parentId, code, key, value, sort, remark);
            id = jdbcTemplate.queryForObject("select last_insert_id()", Long.class);
        } else {
            jdbcTemplate.update("update " + table + " set parent_id=?, code=?, dict_key=?, dict_value=?, sort=?, remark=?, update_time=current_timestamp where id=? and is_deleted=0",
                    parentId, code, key, value, sort, remark, id);
        }
    ...
    public void remove(boolean business, Long id) {
        ...
        jdbcTemplate.update("update " + table + " set is_deleted=1, update_time=current_timestamp where id=?", id);
